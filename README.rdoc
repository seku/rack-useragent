...
of Javascript cookie named i.e: "ie6_rules" is saved on user's disk.
Next time user hits application, middleware checks request for inclusion of cookie "ie6_rules".
If it exists unsupported browsers detection is disabled for this user.

config.middleware.use "Rack::UserAgent::Filter", [
  {:browser => "Internet Explorer", :version => "7.0"}
], :force_with_cookie => "ie6_rules"
